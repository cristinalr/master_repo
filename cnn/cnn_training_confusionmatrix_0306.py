...
from torch.nn import Linear, Sequential, BatchNorm1d, Dropout1d
...
def train_model(model, criterion, optimizer, scheduler, num_epochs=25, train_dataloader=None, val_dataloader=None):
        ...
        for epoch in range(num_epochs):
            ...
            for phase, dataloader in [('train', train_dataloader), ('val', val_dataloader)]:
                ...
                if phase == 'train':
                    ...
                    all_labels_val.extend(all_labels)
                    all_preds_val.extend(all_preds)
    ...
    # Compute the final confusion matrix for the validation set
    conf_matrix = confusion_matrix(all_labels_val, all_preds_val)
    tn, fp, fn, tp = conf_matrix.ravel()
    total_cases = tn + fp + fn + tp

    # Print the total number of cases and the number of cases in each category
...
if __name__ == '__main__':
    ...
    model_conv, metrics = train_model(
        model_conv, criterion, optimizer_conv, exp_lr_scheduler, num_epochs=25,
        train_dataloader=dataloaders['train'], val_dataloader=dataloaders['val']
    )
    ...
    metrics_dict = {
        ...
        'loss_val': metrics['loss_val'][-1]
    }
    metrics_df = pd.DataFrame(metrics_dict)
    metrics_df.to_csv(os.path.join(path_save, 'metrics_testing_cnn_0306.csv'), index=False)
